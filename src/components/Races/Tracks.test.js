...
//   const { getByText } = render(<Tracks user={data} />);
//   expect(getByText(/Season:2005 Champion/)).toBeInTheDocument();
// });

it("TrackItem should have circuit text", () => {
  const { getByTestId } = render(<Tracks racer={data} key={0} championID="alonso" />);
  expect(getByTestId("circuit-text")).toHaveTextContent("Albert Park Grand Prix Circuit");
});

it("TrackItem should have given text", () => {
  const { getByTestId } = render(<Tracks racer={data} key={0} championID="alonso"  />);
  expect(getByTestId("given-name-text")).toHaveTextContent("Giancarlo Fisichella");
});
